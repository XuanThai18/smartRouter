...
const NAV = [
  ...
  },
  {
    group: "Tối ưu hóa",
    items: [
      { href: "/optimize", icon: Zap, label: "Chạy NSGA-II" },
    ],
  },
];
...
export default function Sidebar() {
  ...
  return (
    ...
    >
      {/* Brand */}
      <div style={{ borderBottom: "1px solid hsl(var(--border))" }} className="flex items-center gap-2.5 px-5 h-14">
        <div className="w-7 h-7 rounded-md bg-[hsl(var(--primary))] flex items-center justify-center">
          <Route className="w-4 h-4 text-white" />
        </div>
        <span className="font-semibold text-[hsl(var(--text))] text-sm tracking-tight">SmartRoute</span>
      </div>
      ...
      {/* User */}
      <div style={{ borderTop: "1px solid hsl(var(--border))" }} className="px-3 py-3">
        <div className="flex items-center gap-2.5 px-2.5 py-2 rounded-[var(--radius)] hover:bg-[hsl(var(--bg-hover))] cursor-pointer transition-colors">
          <div className="w-7 h-7 rounded-full bg-[hsl(var(--primary))] flex items-center justify-center text-xs font-bold text-white shrink-0">A</div>
          <div className="flex-1 min-w-0">
            <p className="text-xs font-medium text-[hsl(var(--text))] truncate">Admin</p>
