...
interface Stop   { name: string; address: string; eta: string; done: boolean }

interface VehicleData {
  id: string; plate: string; driver: string; color: string;
  route: LatLng[]; stops: Stop[];
...
function getPosOnRoute(route:readonly LatLng[], t:number): LatLng {
...
export default function TrackingPage() {
  ...
  const [depot, setDepot] = useState<LatLng>({ lat: 10.7769, lng: 106.7009 });
  ...
  const [algoStats, setAlgoStats] = useState({ feasible: 0, savePct: 0 });
  ...
  useEffect(() => {
    ...
    fetchApi<any[]>("/api/optimize").then(data => {
      ...
      dynVehicles.forEach(v => p[v.id] = 0);
      setProgress(p);
      if (dynVehicles.length > 0) setSelected(dynVehicles[0].id);
  ...
  // Init Leaflet map & draw static layers
  useEffect(()=>{
    if(typeof window==="undefined" || loading || vehicles.length === 0) return;
    ...
    import("leaflet").then(L=>{
      ...
      // Draw each vehicle's path and stops
      vehicles.forEach(v=>{
        ...
        v.stops.forEach((stop,i)=>{
          ...
          const sm = L.marker([wp.lat,wp.lng],{
            ...
            zIndexOffset:100,
          }).addTo(map!).bindPopup(`<b style='color:#fff'>${stop.name}</b><br><span style='color:#94a3b8;font-size:11px'>${stop.address}</span><br><span style='color:${v.color};font-size:11px'>ETA ${stop.eta}</span>`);
        ...
        if(v.id===selected) m.bindPopup(`<b style='color:#fff'>${v.plate}</b><br><span style='color:#94a3b8;font-size:11px'>${v.driver}</span>`).openPopup();
        vMarkers.current.set(v.id,m);
      });
    });
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [loading, vehicles]); // Re-run when data is ready
  ...
  const pos     = getPosOnRoute(sel.route, progress[selected||""] || 0);
    ...
    <div className="relative w-full" style={{height:"calc(100vh - 56px)"}}>
      ...
      <div className="absolute top-4 left-4 z-[400] flex items-center gap-2 fade-in">
        <div className="glass px-4 py-2.5 flex items-center gap-2.5">
          ...
          <span className="font-mono text-sm font-semibold text-[hsl(var(--text))]" suppressHydrationWarning>{time||"--:--:--"}</span>
      ...
      {/* Left panel: Vehicle selector & Search */}
      <div className="absolute bottom-4 left-4 z-[400] w-[260px] glass overflow-hidden fade-in flex flex-col max-h-[calc(100vh-140px)]">
        <div className="p-3 shrink-0" style={{borderBottom:"1px solid var(--glass-border)"}}>
          <div className="relative">
            ...
            <input type="text" placeholder="Tìm đơn hàng, địa chỉ..." 
              value={search} onChange={e=>setSearch(e.target.value)}
              className="w-full bg-white/5 border border-white/10 rounded-[var(--radius)] pl-8 pr-3 py-1.5 text-xs text-[hsl(var(--text))] placeholder:text-[hsl(var(--text-muted))] focus:outline-none focus:border-[hsl(var(--primary))] transition-colors" />
          </div>
        </div>

        {search ? (
          <div className="flex-1 overflow-y-auto divide-y" style={{borderColor:"var(--glass-border)"}}>
            {vehicles.flatMap(v => v.stops.map((s,i) => ({...s, vId: v.id, vPlate: v.plate, vColor: v.color, idx: i })))
              .filter(s => s.name.toLowerCase().includes(search.toLowerCase()) || s.address.toLowerCase().includes(search.toLowerCase()))
              .map((s, i) => (
                <button key={i} onClick={()=>{ setSelected(s.vId); setSearch(""); }}
                  className="w-full text-left px-4 py-2.5 hover:bg-white/5 transition-colors cursor-pointer">
                  <div className="text-xs font-semibold text-[hsl(var(--text))] truncate">{s.name}</div>
                  <div className="text-[10px] text-[hsl(var(--text-muted))] truncate mt-0.5">{s.address}</div>
                  <div className="flex items-center gap-1.5 mt-1.5">
                    <div className="w-1.5 h-1.5 rounded-full" style={{background:s.vColor}}/>
                    <span className="text-[10px] text-[hsl(var(--text-sub))]">Xe {s.vPlate}</span>
                  </div>
                </button>
              ))}
          </div>
          ...
          <>
            <div className="px-4 py-2.5 shrink-0 flex items-center gap-2" style={{borderBottom:"1px solid var(--glass-border)", backgroundColor:"rgba(0,0,0,0.2)"}}>
              ...
              <span className="text-[11px] font-semibold uppercase tracking-wider text-[hsl(var(--text-muted))]">Đội xe ({vehicles.length})</span>
            </div>
            <div className="flex-1 overflow-y-auto divide-y" style={{borderColor:"var(--glass-border)"}}>
              {vehicles.map(v=>{
                ...
                return (
                  <button key={v.id} onClick={()=>setSelected(v.id)}
                    className={`w-full px-4 py-3 text-left transition-colors cursor-pointer
                      ${selected===v.id ? "bg-white/5" : "hover:bg-white/3"}`}>
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full
                        ${done ? "bg-green-500/15 text-green-400" : "bg-blue-500/15 text-blue-400"}`}>
                        {done ? "Xong" : "Đang đi"}
                      </span>
                    </div>
      ...
      {/* Right panel: Selected vehicle detail */}
      <div className="absolute top-4 right-4 bottom-4 z-[400] w-[300px] glass flex flex-col overflow-hidden fade-in" style={{marginTop:52}}>
        <div className="px-4 py-3.5 shrink-0" style={{borderBottom:"1px solid var(--glass-border)"}}>
          <div className="flex items-center justify-between mb-1">
            ...
            <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full
              ${isDone ? "bg-green-500/15 text-green-400" : "bg-blue-500/15 text-blue-400"}`}>
        ...
        <div className="px-4 py-3 shrink-0" style={{borderBottom:"1px solid var(--glass-border)"}}>
          <div className="flex justify-between text-[11px] mb-2">
            <span className="text-[hsl(var(--text-muted))]">Tiến độ chuyến đi</span>
            <span className="font-semibold text-[hsl(var(--text))]">{sel.stops.filter((s,i) => s.done || (progress[selected||""]||0) >= ((i+1)/(sel.stops.length+1))).length}/{sel.stops.length} điểm</span>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              {label:"Quãng đường", value:`${sel.totalKm} km`},
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-4 space-y-1">
          {sel.stops.map((stop,i)=>{
            ...
            return (
              <div key={i} className={`rounded-[var(--radius)] px-3 py-2.5 flex items-start gap-2.5 transition-colors
                ${stopDone ? "opacity-50" : "bg-white/4"}`}>
                <div className="mt-0.5 shrink-0">
                  {stopDone
                    ? <CheckCircle2 className="w-3.5 h-3.5 text-[hsl(var(--green))]"/>
                    : <Circle       className="w-3.5 h-3.5 text-[hsl(var(--border))]"/>}
                </div>
        ...
        <div className="px-4 py-3 shrink-0" style={{borderTop:"1px solid var(--glass-border)"}}>
          <div className="flex items-center gap-3 flex-wrap">
            {vehicles.map(v=>(
              <button key={v.id} onClick={()=>setSelected(v.id)}
                className="flex items-center gap-1.5 text-[10px] text-[hsl(var(--text-muted))] hover:text-[hsl(var(--text))] transition-colors cursor-pointer">
      ...
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-[400] fade-in">
